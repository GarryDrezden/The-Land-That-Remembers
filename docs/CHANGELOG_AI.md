...
**Сделано:**
- bake: толщина карниза, binary alpha (без soft fringe), отказ от crushed-black сэмплов (не чёрные плиты);
- старт-камера: zoom **1.4**, offset `(0,-40)` — дом целиком в кадре с воздухом сверху и двором снизу;
- скрины обновлены.

**Не делалось:** перерисовка арта в `upload/`, смена героя/движения.

---

## 2026-08-02 (a) — fix VS01 house green bleed + start composition

**Сделано:**
- runtime bake `main_house_v1`: закрыты щели под карнизом (не трогая `upload/`);
- display scale ≈ **0.88** nearest после ÷4 (≈ −12%);
- дом выше на карте, северный забор отодвинут, spawn на переднем дворе;
- стартовая камера (позже уточнена в 02b): общий план дома + дорожка + двор;
- скрины `yard_vs01_start.png` и обновлённые door/overview.

**Не делалось:** механика движения/герой, новые системы, перерисовка дома.
